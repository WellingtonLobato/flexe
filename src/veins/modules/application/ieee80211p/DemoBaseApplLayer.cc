...
std::vector<std::string> DemoBaseApplLayer::split(std::string str,std::string sep){
...
void DemoBaseApplLayer::sendCloud(cMessage* msg, std::string moduleName){
    cModule *directModule = getModuleByPath(moduleName.c_str());
    sendDirect(msg, directModule, "veinsradioIn");
}
...
void DemoBaseApplLayer::handleLowerMsg(cMessage* msg){
    ...
    //ASSERT(wsm); //ERROR WITH LOW LAYER MSG
