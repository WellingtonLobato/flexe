...
class fixed_sized_cache
{
  public:
    ...
    /*
    std::multimap<std::size_t, Key> GetFrequency(){
        std::multimap<std::size_t, Key> keyVector;
        //keyVector = cache_policy.GetFrequency();
        return keyVector;
    }
    */
    ...
    std::vector<Key> GetKeys(){
        ...
        for(auto kv : cache_items_map) {
            keyVector.push_back(kv.first);
        }
        return keyVector;
    }
