...
class FlexeClient {
    ...
    int evaluate(int idVehicle, const std::string& dataset, const std::string& modelName, int batchSize){
        TTRequest evaluateRequest;
        evaluateRequest.set_idvehicle(idVehicle+1);
        evaluateRequest.set_datasetpath(dataset);
    ...
    ModelReply fit(int idVehicle, const std::string& dataset, const std::string& modelName, int epochs, int batchSize){
        TTRequest fitRequest;
        fitRequest.set_idvehicle(idVehicle+1);
        fitRequest.set_datasetpath(dataset);
    ...
    int update_model(int idVehicle, const std::string& dataset, const std::string& modelName, int epochs, int batchSize){
        TTRequest fitRequest;
        fitRequest.set_idvehicle(idVehicle+1);
        fitRequest.set_datasetpath(dataset);
        fitRequest.set_modelname(modelName);
