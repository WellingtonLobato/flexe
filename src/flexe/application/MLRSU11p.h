...
class FlexeClient {
    ...
    double aggregate_evaluate(int idRSU, bool trainFlag, const std::string& modelName, int batchSize){
        ...
        if(status.ok()){
            std::cout << "Aggregate Evaluation requested successfully!" << endl;
            return reply.loss();
        } else {
            std::cout << status.error_code() << ": " << status.error_message() << std::endl;
            return -1;
    ...
    ModelReply aggregate_fit(ModelRequest modelGRPC){
        ...
        if(status.ok()){
            std::cout << "Aggregate requested successfully!" << endl;
        } else {
            std::cout << status.error_code() << ": " << status.error_message() << std::endl;
        }
        return reply;
...
class MLRSU11p : public DemoBaseApplLayer {
public:
    ~MLRSU11p();
...
protected:
    ...
    std::map<int, std::tuple<std::string, std::vector<int>, int>> modelsMap;
    std::map<int, std::tuple<std::string, std::vector<int>, int>>::iterator it;
