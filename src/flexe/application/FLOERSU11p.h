...
//lfu_cache_t<int, std::string> cache(CACHE_SIZE);
//lru_cache_t<int, std::string> cache(CACHE_SIZE);
fifo_cache_t<int, std::string> cache(CACHE_SIZE);
...
class FLOERSU11p : public DemoBaseApplLayer {
public:
    ~FLOERSU11p();
...
protected:
    ...
    double meanContactTime(std::map<int, std::tuple<double, double>> contactMap);
